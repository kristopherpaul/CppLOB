...
public:
    ...
    constexpr std::optional<spec::Price> best_price(std::size_t num_levels) const noexcept {
        if constexpr (Side == spec::Side::Buy) {
            for (std::size_t i = L0_Words; i-- > 0;) {
                if (i >= L0_Words) continue;
...
class Stage6_SideSpecialized {
    ...
    template <spec::Side Side>
    constexpr auto& get_ladder() noexcept {
        if constexpr (Side == spec::Side::Buy) return bids_;
        else return asks_;
    }

    template <spec::Side Side>
    constexpr auto& get_bitmap() noexcept {
        if constexpr (Side == spec::Side::Buy) return bid_bitmap_;
        else return ask_bitmap_;
    }

    template <spec::Side Side>
    constexpr auto& get_opposite_ladder() noexcept {
        if constexpr (Side == spec::Side::Buy) return asks_;
        else return bids_;
    }

    template <spec::Side Side>
    constexpr auto& get_opposite_bitmap() noexcept {
        if constexpr (Side == spec::Side::Buy) return ask_bitmap_;
        else return bid_bitmap_;
    }
...
public:
    ...
    bool submit(const spec::OrderInput& in, TradeSink& sink) noexcept {
        ...
        if (in.side == spec::Side::Buy) {
            return match<spec::Side::Buy>(in, pool_idx, sink);
        } else {
            return match<spec::Side::Sell>(in, pool_idx, sink);
        }
    }
    ...
    bool cancel(spec::OrderId id) noexcept {
        ...
        auto& ladder = get_ladder<side>();
        auto& bitmap = get_bitmap<side>();
    ...
    std::optional<spec::Price> best_bid() const noexcept { 
        return bid_bitmap_.template best_price<spec::Side::Buy>(NumLevels); 
    }
    std::optional<spec::Price> best_ask() const noexcept { 
        return ask_bitmap_.template best_price<spec::Side::Sell>(NumLevels); 
    }
...
private:
    template <spec::Side Side>
    bool match(const spec::OrderInput& in, std::uint32_t pool_idx, TradeSink& sink) noexcept {
        auto& node = pool_[pool_idx];
        spec::Quantity remaining = in.quantity;

        while (remaining > 0) {
            auto opp_best = get_opposite_bitmap<Side>().template best_price<spec::opposite(Side)>(NumLevels);
            if (!opp_best) break;
            if (!spec::crosses(Side, in.price, *opp_best)) break;

            spec::Price opp_price = *opp_best;
            std::size_t opp_idx = price_to_index(opp_price);
            auto& level = get_opposite_ladder<Side>()[opp_idx];
            std::uint32_t resting_idx = level.head;
            if (resting_idx == UINT32_MAX) {
                get_opposite_bitmap<Side>().clear(opp_idx);
                continue;
            }
            ...
            if (resting.quantity == 0) {
                ...
                if (level.empty()) get_opposite_bitmap<Side>().clear(opp_idx);
            }
        }

        if (remaining > 0) {
            node.quantity = remaining;
            auto& level = get_ladder<Side>()[price_to_index(in.price)];
            append_to_level(level, pool_idx);
            level.total_quantity += remaining;
            get_bitmap<Side>().set(price_to_index(in.price));
        } else {
            index_.erase(in.id);
            pool_.deallocate(pool_idx);
        }
